
using System;


namespace lesson1.Solutions
{
    class Lab2
    {
        public void Problem1()
        {
            System.Console.WriteLine("Yechim 1:");
            System.Console.WriteLine($"{222222}");
            System.Console.WriteLine($"{2+"    "+2}");
            System.Console.WriteLine($"{2+"    "+2}");
            System.Console.WriteLine($"{222222}");
            
        }

        public void Problem2()
        {
            System.Console.WriteLine("Yechim 2:");
            System.Console.WriteLine("  A");
            System.Console.WriteLine(" A A ");
            System.Console.WriteLine("AAAAA");
            
            
        }
        public void Problem3()
        {
            System.Console.WriteLine("Yechim 3");
            Console.WriteLine("Birthdate");
            Console.Write("Enter date: ");
            var date = System.Console.ReadLine();
            Console.Write("Enter month: ");
            var month = Console.ReadLine();

            string Date = date.ToString().PadLeft(2, '0');
            string Month = month.ToString().PadLeft(2, '0');

            System.Console.WriteLine("Birthday is: " + Date + "-" + Month + " " + "(mm-dd)");
        }
        
        public void Problem4()
        {
            System.Console.WriteLine("Yechim 4:");
            System.Console.Write("Enter month and date: ");
            var line = Console.ReadLine();
            var data = line.Split(' ');
            var a = int.Parse(data[0]);
            var b = int.Parse(data[1]);

            string A = a.ToString().PadLeft(2, '0');
            string B = b.ToString().PadLeft(2, '0');

            System.Console.WriteLine("Birthday is: " + A + "-" + B + " " + "(mm-dd)");

        }
        public void Problem5()
        {
            System.Console.WriteLine("Yechim 5:");
            System.Console.WriteLine("Enter the number between 1 and 10 ");
            var n = Console.ReadLine();
            System.Console.WriteLine($"{n+n+n+n+n+n}");
            System.Console.WriteLine($"{n+"    "+n}");
            System.Console.WriteLine($"{n+"    "+n}");
            System.Console.WriteLine($"{n+n+n+n+n+n}");

        }
        public void Problem6()
        {
            System.Console.WriteLine("Yechim 6:");
            System.Console.WriteLine("Sun Mon Tue Wed Thu Fri Sat");
            System.Console.WriteLine("23  24  25  26  27  28  1");
        }
        public void Problem7()
        {
            System.Console.WriteLine("Yechim 7:");
            System.Console.WriteLine("Enter any number ");
            var line = Console.ReadLine();
            var data = line.Split(' ');
            var n = int.Parse(data[0]);
            System.Console.WriteLine($"{n} * 1 = {n * 1}");
            System.Console.WriteLine($"{n} * 2 = {n * 2}");
            System.Console.WriteLine($"{n} * 3 = {n * 3}");
            System.Console.WriteLine($"{n} * 4 = {n * 4}");
            System.Console.WriteLine($"{n} * 5 = {n * 5}");
            System.Console.WriteLine($"{n} * 6 = {n * 6}");
            System.Console.WriteLine($"{n} * 7 = {n * 7}");
            System.Console.WriteLine($"{n} * 8 = {n * 8}");
            System.Console.WriteLine($"{n} * 9 = {n * 9}");
            

        }

        
        public void Problem8()
        {
            System.Console.WriteLine("Yechim 8");
            System.Console.WriteLine($" 1! = {1}");
            System.Console.WriteLine($" 2! = {1 * 2}");
            System.Console.WriteLine($" 3! = {1 * 2 * 3}");
            System.Console.WriteLine($" 4! = {1 * 2 * 3 * 4}");
            System.Console.WriteLine($" 5! = {1 * 2 * 3 * 4 * 5}");
        }
       
        public void Problem9()
        {
            System.Console.WriteLine("Yechim 9:");
            int a = 0;
            int b = 1;
            int c = a + b;

            System.Console.Write($"{a} {b} {c} ");

            a = b;
            b = c;
            c = a+b;

            System.Console.Write($"{c} ");  

            a = b;
            b = c;
            c = a+b;

            System.Console.Write($"{c} ");      

            a = b;
            b = c;
            c = a+b;

            System.Console.Write($"{c} ");     

            a = b;
            b = c;
            c = a+b;

            System.Console.Write($"{c} ");    

            a = b;
            b = c;
            c = a+b;

            System.Console.Write($"{c} ");     

            a = b;
            b = c;
            c = a+b;

            System.Console.Write($"{c} ");      

            a = b;
            b = c;
            c = a+b;

            System.Console.Write($"{c} ");  
            
        }


    }
}